...
durka_contact_details_new (void)
{
  return g_slice_new0(DurkaContactDetails);
}

static void
durka_contact_details_destroy (gpointer p)
{
  DurkaContactDetails *d = p;

  tp_clear_pointer (&d->tags, g_hash_table_unref);

  g_free (d->alias);
  g_slice_free (DurkaContactDetails, d);
}

static void
mutable_contact_list_iface_init (TpMutableContactListInterface *);
static void
blockable_contact_list_iface_init (TpBlockableContactListInterface *);
static void
contact_group_list_iface_init (TpContactGroupListInterface *);
static void
...
static void
durka_contact_list_close_all (DurkaContactList *self)
{
  ...
  tp_clear_pointer (&self->priv->cancelled_publish_requests, tp_handle_set_destroy);
  ...
  if (self->priv->status_changed_id != 0) {
    g_signal_handler_disconnect (self->priv->conn, self->priv->status_changed_id);
...
static DurkaContactDetails *
lookup_contact (DurkaContactList *self,
                TpHandle contact)
{
  return g_hash_table_lookup (self->priv->contact_details, GUINT_TO_POINTER (contact));
}

static DurkaContactDetails *
ensure_contact (DurkaContactList *self,
                TpHandle contact,
                gboolean *created)
{
  DurkaContactDetails *ret = lookup_contact (self, contact);

  if (ret == NULL) {
    tp_handle_set_add (self->priv->contacts, contact);

    ret = durka_contact_details_new ();
    ret->alias = g_strdup (tp_handle_inspect (self->priv->contact_repo, contact));

    g_hash_table_insert (self->priv->contact_details, GUINT_TO_POINTER (contact), ret);
     ...
     * list */
    if (created != NULL) {
      *created = (g_hash_table_lookup (self->priv->publish_requests, GUINT_TO_POINTER (contact)) == NULL);
    }
...
static gchar *
ensure_tag (DurkaContactList *self,
            const gchar *s,
            gboolean emit_signal)
{
  ...
  if (r == NULL) {
    ...
    if (emit_signal)
      tp_base_contact_list_groups_created ((TpBaseContactList *) self, &s, 1);
  }
...
static void
durka_contact_list_set_contact_groups_async (TpBaseContactList *contact_list,
                                             TpHandle contact,
                                             ...
                                             gpointer user_data)
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
  gboolean created;
  gsize i;
  DurkaContactDetails *d;
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_set_contact_groups_async);
}
...
receive_contact_lists (gpointer p)
{
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "sjoerd@durka.com", NULL, NULL);
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "guillaume@durka.com", NULL, NULL);
  d = ensure_contact (self, handle, NULL);
  g_free (d->alias);
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "olivier@durka.com", NULL, NULL);
  d = ensure_contact (self, handle, NULL);
  g_free (d->alias);
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "travis@durka.com", NULL, NULL);
  d = ensure_contact (self, handle, NULL);
  g_free (d->alias);
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "geraldine@durka.com", NULL, NULL);
  d = ensure_contact (self, handle, NULL);
  g_free (d->alias);
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "helen@durka.com", NULL, NULL);
  d = ensure_contact (self, handle, NULL);
  g_free (d->alias);
  ...
  handle = tp_handle_ensure (self->priv->contact_repo, "wim@durka.com", NULL, NULL);
  tp_handle_set_add (self->priv->contacts, handle);
  g_hash_table_insert (self->priv->publish_requests, GUINT_TO_POINTER (handle),
                       g_strdup ("I'm more metal than you!"));

  handle = tp_handle_ensure (self->priv->contact_repo, "christian@durka.com", NULL, NULL);
  tp_handle_set_add (self->priv->contacts, handle);
  g_hash_table_insert (self->priv->publish_requests, GUINT_TO_POINTER (handle),
                       g_strdup ("I have some fermented herring for you"));

  /* Add a couple of blocked contacts. */
  handle = tp_handle_ensure (self->priv->contact_repo, "bill@durka.com", NULL, NULL);
  tp_handle_set_add (self->priv->blocked_contacts, handle);
  handle = tp_handle_ensure (self->priv->contact_repo, "steve@durka.com", NULL, NULL);
  tp_handle_set_add (self->priv->blocked_contacts, handle);
                   ...
                   DurkaContactList *self)
{
  switch (status) {
    case TP_CONNECTION_STATUS_CONNECTED: {
      ...
      g_timeout_add_full (G_PRIORITY_DEFAULT, 2 * self->priv->simulation_delay, receive_contact_lists,
...
constructed (GObject *object)
{
  ...
  void
  (*chain_up) (GObject *) =
  ((GObjectClass *) durka_contact_list_parent_class)->constructed;

  if (chain_up != NULL) {
    chain_up (object);
  }

  g_object_get (self, "connection", &self->priv->conn, NULL);
  g_assert (self->priv->conn != NULL);

  self->priv->contact_repo = tp_base_connection_get_handles (self->priv->conn, TP_HANDLE_TYPE_CONTACT);
  ...
  self->priv->cancelled_publish_requests = tp_handle_set_new (self->priv->contact_repo);

  self->priv->status_changed_id = g_signal_connect (self->priv->conn, "status-changed",
                                                   (GCallback ) status_changed_cb, self);
}

static void
send_updated_roster (DurkaContactList *self,
                     TpHandle contact)
{
  DurkaContactDetails *d = g_hash_table_lookup (self->priv->contact_details, GUINT_TO_POINTER (contact));
  const gchar *request = g_hash_table_lookup (self->priv->publish_requests, GUINT_TO_POINTER (contact));
  const gchar *identifier = tp_handle_inspect (self->priv->contact_repo, contact);
  ...
  if (d == NULL) {
    ...
    g_message ("\tcan see our presence = %s",
              d->publish ? "yes" : (request != NULL ? "no, but has requested it" : "no"));
    g_message (
        "\tsends us presence = %s",
        d->subscribe ?
            "yes" :
...
static void
durka_contact_list_set_group_members_async (TpBaseContactList *contact_list,
                                            const gchar *group,
                                            TpHandleSet *contacts,
                                            GAsyncReadyCallback callback,
                                            gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    ...
    DurkaContactDetails *d = ensure_contact (self, member, &created);
  ...
  if (!tp_handle_set_is_empty (removed))
    tp_base_contact_list_groups_changed (contact_list, removed, NULL, 0, &group, 1);

  tp_handle_set_destroy (added);
  tp_handle_set_destroy (removed);
  tp_handle_set_destroy (new_contacts);
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_set_group_members_async);
}

static void
durka_contact_list_add_to_group_async (TpBaseContactList *contact_list,
                                       const gchar *group,
                                       TpHandleSet *contacts,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data)
{
  ...
  tp_handle_set_destroy (new_contacts);
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_add_to_group_async);
}

static void
durka_contact_list_remove_from_group_async (TpBaseContactList *contact_list,
                                            const gchar *group,
                                            TpHandleSet *contacts,
                                            GAsyncReadyCallback callback,
                                            gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    DurkaContactDetails *d = lookup_contact (self, member);
  ...
  if (!tp_handle_set_is_empty (changed))
    tp_base_contact_list_groups_changed (contact_list, changed, NULL, 0, &group, 1);

  tp_handle_set_destroy (changed);
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_remove_from_group_async);
}
...
self_and_contact_new (DurkaContactList *self,
                      TpHandle contact)
{
...
static void
receive_auth_request (DurkaContactList *self,
                      TpHandle contact)
{
  DurkaContactDetails *d;
  ...
  if (d != NULL && d->pre_approved) {
    ...
    d->publish = TRUE;
    g_hash_table_remove (self->priv->publish_requests, GUINT_TO_POINTER (contact));
    tp_handle_set_remove (self->priv->cancelled_publish_requests, contact);
    send_updated_roster (self, contact);
  } else {
    tp_handle_set_add (self->priv->contacts, contact);
    g_hash_table_insert (self->priv->publish_requests, GUINT_TO_POINTER (contact),
                         g_strdup ("May I see your presence, please?"));
  }

  tp_base_contact_list_one_contact_changed ((TpBaseContactList *) self, contact);
  ...
  if (strstr (tp_handle_inspect (self->priv->contact_repo, contact), "@cancel.") != NULL) {
...
receive_authorized (gpointer p)
{
  ...
  tp_base_contact_list_one_contact_changed ((TpBaseContactList *) s->self, s->contact);
...
durka_contact_list_get_presence (DurkaContactList *self,
                                 TpHandle contact)
{
  DurkaContactDetails *d = lookup_contact (self, contact);
  const gchar *id;

  if (d == NULL || !d->subscribe) {
    /* we don't know the presence of people not on the subscribe list,
     * by definition */
    return DURKA_CONTACT_LIST_PRESENCE_UNKNOWN;
  }

  id = tp_handle_inspect (self->priv->contact_repo, contact);

  /* In this durka CM, we fake contacts' presence based on their name:
  ...
  if ((id[0] >= 'A' && id[0] <= 'M') || (id[0] >= 'a' && id[0] <= 'm')) {
    return DURKA_CONTACT_LIST_PRESENCE_AVAILABLE;
  }

  return DURKA_CONTACT_LIST_PRESENCE_AWAY;
}
...
static TpHandleSet *
durka_contact_list_dup_contacts (TpBaseContactList *contact_list)
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
...
static TpHandleSet *
durka_contact_list_dup_group_members (TpBaseContactList *contact_list,
                                      const gchar *group)
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
  TpIntsetFastIter iter;
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    ...
    if (d != NULL && d->tags != NULL && g_hash_table_lookup_extended (d->tags, group, NULL, NULL))
...
static const DurkaContactDetails no_details = {
NULL,
FALSE,
FALSE,
FALSE,
...
static void
durka_contact_list_dup_states (TpBaseContactList *contact_list,
                               TpHandle contact,
...
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
  const DurkaContactDetails *details = lookup_contact (self, contact);
  const gchar *request = g_hash_table_lookup (self->priv->publish_requests, GUINT_TO_POINTER (contact));
  ...
  if (subscribe != NULL)
    *subscribe = compose_presence (details->subscribe, details->subscribe_requested,
                                   details->subscribe_rejected);

  if (publish != NULL)
    *publish = compose_presence (details->publish, (request != NULL),
                                 tp_handle_set_is_member (self->priv->cancelled_publish_requests, contact));
...
static void
durka_contact_list_request_subscription_async (TpBaseContactList *contact_list,
                                               TpHandleSet *contacts,
                                               const gchar *message,
                                               GAsyncReadyCallback callback,
                                               gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    ...
    g_message ("Transmitting authorization request to %s: %s",
              tp_handle_inspect (self->priv->contact_repo, member), message);
     ...
     * and allows or rejects it. In this durka connection manager,
    ...
    if (message[0] == '\0' || strstr (message_lc, "please") != NULL) {
      g_timeout_add_full (G_PRIORITY_LOW, self->priv->simulation_delay, receive_authorized,
                          self_and_contact_new (self, member), self_and_contact_destroy);
    } else {
      g_timeout_add_full (G_PRIORITY_LOW, self->priv->simulation_delay, receive_unauthorized,
                          self_and_contact_new (self, member), self_and_contact_destroy);
    }
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_request_subscription_async);
}

static void
durka_contact_list_authorize_publication_async (TpBaseContactList *contact_list,
                                                TpHandleSet *contacts,
                                                GAsyncReadyCallback callback,
                                                gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    DurkaContactDetails *d = ensure_contact (self, member, NULL);
    const gchar *request = g_hash_table_lookup (self->priv->publish_requests, GUINT_TO_POINTER (member));

    if (tp_handle_set_remove (self->priv->cancelled_publish_requests, member))
      tp_handle_set_add (changed, member);
    ...
    if (request == NULL) {
      ...
      d->publish = TRUE;
      g_hash_table_remove (self->priv->publish_requests, GUINT_TO_POINTER (member));
      send_updated_roster (self, member);
      tp_handle_set_add (changed, member);
    }
  }
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_authorize_publication_async);
}

static void
durka_contact_list_store_contacts_async (TpBaseContactList *contact_list,
                                         TpHandleSet *contacts,
                                         GAsyncReadyCallback callback,
                                         gpointer user_data)
{
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_store_contacts_async);
}

static void
durka_contact_list_remove_contacts_async (TpBaseContactList *contact_list,
                                          TpHandleSet *contacts,
                                          GAsyncReadyCallback callback,
                                          gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    ...
    if (lookup_contact (self, member) != NULL
        || g_hash_table_lookup (self->priv->publish_requests, GUINT_TO_POINTER (member)) != NULL
        || tp_handle_set_is_member (self->priv->cancelled_publish_requests, member)) {
      tp_handle_set_add (removed, member);

      g_hash_table_remove (self->priv->contact_details, GUINT_TO_POINTER (member));
      g_hash_table_remove (self->priv->publish_requests, GUINT_TO_POINTER (member));
      tp_handle_set_remove (self->priv->contacts, member);
      tp_handle_set_remove (self->priv->cancelled_publish_requests, member);
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_remove_contacts_async);
}

static void
durka_contact_list_unsubscribe_async (TpBaseContactList *contact_list,
                                      TpHandleSet *contacts,
                                      GAsyncReadyCallback callback,
                                      gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    ...
    if (d != NULL) {
      if (d->subscribe_requested) {
        ...
        g_message ("We no longer want presence from %s", tp_handle_inspect (self->priv->contact_repo, member));
        d->subscribe = FALSE;
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_unsubscribe_async);
}

static void
durka_contact_list_unpublish_async (TpBaseContactList *contact_list,
                                    TpHandleSet *contacts,
                                    GAsyncReadyCallback callback,
                                    gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    ...
    if (d != NULL) {
      ...
      if (d->publish) {
        g_message ("Removing authorization from %s", tp_handle_inspect (self->priv->contact_repo, member));
        d->publish = FALSE;
        tp_handle_set_add (changed, member);
        send_updated_roster (self, member);
        ...
        g_timeout_add_full (G_PRIORITY_LOW, self->priv->simulation_delay, auth_request_cb,
                            self_and_contact_new (self, member), self_and_contact_destroy);
      }
    }
  }
  ...
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_unpublish_async);
}

static TpHandleSet *
durka_contact_list_dup_blocked_contacts (TpBaseContactList *contact_list)
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
...
static void
durka_contact_list_block_contacts_with_abuse_async (TpBaseContactList *contact_list,
                                                    TpHandleSet *contacts,
                                                    ...
                                                    gpointer user_data)
{
  ...
  while (tp_intset_fast_iter_next (&iter, &member)) {
    if (!tp_handle_set_is_member (self->priv->blocked_contacts, member)) {
      g_message ("Adding contact %s to blocked list", tp_handle_inspect (self->priv->contact_repo, member));
  ...
  tp_handle_set_destroy (changed);
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_block_contacts_with_abuse_async);
}

static void
durka_contact_list_unblock_contacts_async (TpBaseContactList *contact_list,
                                           TpHandleSet *contacts,
                                           GAsyncReadyCallback callback,
                                           gpointer user_data)
{
  ...
  tp_handle_set_destroy (changed);
  tp_simple_async_report_success_in_idle ((GObject *) self, callback, user_data,
                                          durka_contact_list_unblock_contacts_async);
}

static guint
durka_contact_list_get_group_storage (TpBaseContactList *contact_list G_GNUC_UNUSED)
{
  return TP_CONTACT_METADATA_STORAGE_TYPE_ANYONE;
}

static GStrv
durka_contact_list_dup_groups (TpBaseContactList *contact_list)
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
  GPtrArray *tags = g_ptr_array_sized_new (g_hash_table_size (self->priv->all_tags) + 1);
  GHashTableIter iter;
  gpointer tag;
...
static GStrv
durka_contact_list_dup_contact_groups (TpBaseContactList *contact_list,
                                       TpHandle contact)
{
  DurkaContactList *self = DURKA_CONTACT_LIST (contact_list);
  GPtrArray *tags = g_ptr_array_sized_new (g_hash_table_size (self->priv->all_tags) + 1);
...
static void
durka_contact_list_remove_group_async (TpBaseContactList *contact_list,
                                       const gchar *group,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data)
{
  ...
  tp_simple_async_report_success_in_idle ((GObject *) contact_list, callback, user_data,
                                          durka_contact_list_remove_group_async);
}

static gchar *
durka_contact_list_normalize_group (TpBaseContactList *contact_list,
                                    const gchar *id)
{
  if (id[0] == '\0')
    return NULL;
...
static void
durka_contact_list_rename_group_async (TpBaseContactList *contact_list,
                                       ...
                                       gpointer user_data)
{
  ...
  tp_simple_async_report_success_in_idle ((GObject *) contact_list, callback, user_data,
                                          durka_contact_list_rename_group_async);
}
...
mutable_contact_list_iface_init (TpMutableContactListInterface *iface)
{
  ...
  iface->request_subscription_async = durka_contact_list_request_subscription_async;
  iface->authorize_publication_async = durka_contact_list_authorize_publication_async;
  iface->store_contacts_async = durka_contact_list_store_contacts_async;
  iface->remove_contacts_async = durka_contact_list_remove_contacts_async;
  iface->unsubscribe_async = durka_contact_list_unsubscribe_async;
  iface->unpublish_async = durka_contact_list_unpublish_async;
}
...
blockable_contact_list_iface_init (TpBlockableContactListInterface *iface)
{
  ...
  iface->dup_blocked_contacts = durka_contact_list_dup_blocked_contacts;
  iface->block_contacts_with_abuse_async = durka_contact_list_block_contacts_with_abuse_async;
  iface->unblock_contacts_async = durka_contact_list_unblock_contacts_async;
}

static void
contact_group_list_iface_init (TpContactGroupListInterface *iface)
{
  iface->dup_groups = durka_contact_list_dup_groups;
  iface->dup_group_members = durka_contact_list_dup_group_members;
  iface->dup_contact_groups = durka_contact_list_dup_contact_groups;
  iface->normalize_group = durka_contact_list_normalize_group;
}

static void
mutable_contact_group_list_iface_init (TpMutableContactGroupListInterface *iface)
{
  iface->set_group_members_async = durka_contact_list_set_group_members_async;
  iface->add_to_group_async = durka_contact_list_add_to_group_async;
  iface->remove_from_group_async = durka_contact_list_remove_from_group_async;
  iface->remove_group_async = durka_contact_list_remove_group_async;
  iface->rename_group_async = durka_contact_list_rename_group_async;
  iface->set_contact_groups_async = durka_contact_list_set_contact_groups_async;
